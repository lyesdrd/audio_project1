...
public class AudioIO2 {
   ...
   public static TargetDataLine obtainAudioInput(String mixerName, int sampleRate) throws LineUnavailableException {
       Mixer.Info mixerinfo = AudioIO2.getMixerInfo(mixerName);
...
*/
    ...
    public static Thread createThread(String inputMixer, String outputMixer, int sampleRate, int frameSize,AudioProcessor as) throws LineUnavailableException {
        try{
        ...
        SourceDataLine outLine = obtainAudioOutput(outputMixer, sampleRate);
         as.setAudioProcessor(inLine, outLine, frameSize);
        ...
        return t;}
            ...
            inLine.open(); inLine.start(); outLine.open(); outLine.start();
            Thread t = new  Thread(as);
            System.out.println("Line non valid mise a default des paramettres");
            return t;
        }
    }
    ...
    public static void stopAudioProcessing(ThreadHandler t, AudioProcessor as){
        try {
            t.stopThreadSon(as);
            as.getAudioInput().close();
            as.getAudioOutput().close();
